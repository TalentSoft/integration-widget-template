...
export class Search extends React.Component<SearchProps, SearchState> {
  ...
  constructor(props: SearchProps) {
    ...
    this.handleSearchChangeThrottled = throttle(300, (e) => {
  ...
  componentWillReceiveProps(nextProps: SearchProps) {
    if (
      nextProps.status !== this.state.statusAvaible &&
      nextProps.status !== undefined
    ) {
      const statusToShow = nextProps.status.map((item, index) => ({
        value: item.value,
        show:
          this.state.statusAvaible[index] != undefined
            ? this.state.statusAvaible[index].show
            : true,
      }));
  ...
  handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    this.setState({ searchText: e.target.value }, () =>
      this.props.handleChangeSearch !== undefined
        ? this.props.handleChangeSearch(
            this.state.searchText,
            this.state.statusAvaible
          )
        : null
    );
  };
  ...
  handleClickStatus(status: StatusAvailable) {
    ...
    this.setState({ statusAvaible: allStatus }, () =>
      this.props.handleChangeSearch(
        this.state.searchText,
        this.state.statusAvaible
      )
  ...
  render() {
    ...
    if (this.state.statusAvaible !== undefined) {
      statusAvailable = (
        <div className="status-available">
          {this.displayStatus(this.state.statusAvaible)}
        </div>
      );
    }
    ...
    return (
      <AnimateHeight
        height={this.props.isVisible ? 'auto' : 0}
      ...
      >
        <div className="input-search">
          <input
            ...
            onChange={(e) => {
              e.persist();
              this.handleSearchChangeThrottled(e);
            }}
          />
          {statusAvailable}
        </div>
