...
export class NormalWidget extends React.Component<
  NormalWidgetProps,
  NormalWidgetState
> {
  ...
  componentDidMount() {
    this.getData().catch((r) => {
      this.props.widgetProps.myTSHostService.raiseError(
        'could not load data',
        'ERR_SERVICE',
        r
      );
  ...
  public async getData() {
    ...
    const response = await myTSHostService.requestExternalResource({
      verb: 'GET',
      url: this.props.widgetProps.params['domain'],
    });
  ...
  handleChangeSearch(textToSearch: string, status: StatusAvailable[]) {
    ...
    const availableStatus = status.map((stat) => {
      if (stat.show) {
    ...
    if (textToSearch !== '' || availableStatus.indexOf('') > -1) {
      data = data.filter(function (item) {
        if (
          item.name
            .toLocaleUpperCase()
            .search(textToSearch.toLocaleUpperCase()) === -1
        ) {
          return false;
        }
  ...
  getAllAvailableStatus() {
    let allStatus: StatusAvailable[] = this.state.data.map((item) => {
      return { value: item.status, show: true };
    });
  ...
  formattedDataForList() {
    let items: Line[] = [];
    const dataToUse =
      this.state.searchtextvalue != '' || this.state.searchResult.length != 0
        ? this.state.searchResult
        : this.state.data;

    if (dataToUse !== undefined && dataToUse.length !== 0) {
      dataToUse.map((line, index) =>
        items.push({
          id: index,
          urlPicture: '',
          title: line.name,
  ...
  public render() {
    return (
      <div className="widget-template">
        <Search
          isVisible={this.props.isSearchVisible}
          handleChangeSearch={(textToSearch, status) =>
            this.handleChangeSearch(textToSearch, status)
          }
          status={this.getAllAvailableStatus()}
        />
        <Scrollbars
          autoHeight
          autoHeightMin={this.props.isSearchVisible ? 445 : 495}
        >
          <List
            showPicture={true}
            showDetail={true}
            values={this.formattedDataForList()}
          />
          {/* <PieCharts 
          ...
          <div className="partner-title">
            {
              this.props.widgetProps.myTSHostService.getPreloadedResources()[
                'partner-title'
              ]
            }
          </div>
