...
export class ListEnlarged extends React.Component<
  ListEnlargedProps,
  ListEnlargedState
> {
  constructor(props: ListEnlargedProps) {
    ...
    this.state = {
      selectedItem:
        this.props.values.length !== 0 ? this.props.values[0]!.id : 0,
    };
  }

  displayAvatar(urlPicture: string) {
    const imageLink =
      urlPicture !== undefined && urlPicture !== '' ? (
        <img className="avatar-picture" src={urlPicture}></img>
      ) : (
        <i className="icon-profil"></i>
      );
    return (
      <div className="line-avatar">
        <div className="Hylia-a-avatar Hylia-a-avatar--default Hylia-a-avatar--medium Hylia-a-avatar--initial">
          <div className="Hylia-a-avatar__wrapper">
            <div className="Hylia-a-avatar__picture">
              <span className="Hylia-a-avatar__initial"></span>
              {imageLink}
            </div>
          </div>
        </div>
  ...
  displayDetail(value: Line) {
    return (
      <div
        className={
          'detail' + (this.state.selectedItem === value.id ? ' active' : '')
        }
        key={value.id}
      >
        <Detail {...value} />
  ...
  displayLine(value: Line) {
    const status = this.props.showStatus
      ? this.displayStatus(value.detail)
      : undefined;
    const avatar = this.props.showPicture
    ...
    return (
      <li
        className={
          'list-line' + (this.state.selectedItem === value.id ? ' active' : '')
        }
        onClick={() => this.handleSelectItem(value.id)}
        key={value.id}
      >
        {avatar}
        <div className="line">
